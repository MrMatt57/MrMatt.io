---
allowed-tools: Bash(git *), Bash(ls *), Bash(mkdir *), Bash(cp *), Bash(hugo *), Read, Write, Edit, Glob, Grep, AskUserQuestion
---
...
Determine a short, descriptive kebab-case name for the branch (e.g., `new-post`, `dark-mode-fix`, `add-search`).
...
The spec filename format is: `NNN-{branch-name}.md` (e.g., `001-new-post.md`).
...
```bash
git worktree add ../mrmatt-{name} -b feature/{name} origin/main
```

This creates:
- A new directory `../mrmatt-{name}` (sibling to the main repo)
...
```bash
mkdir -p ../mrmatt-{name}/.specs
cp .specs/NNN-{branch-name}.md ../mrmatt-{name}/.specs/
```

## Step 8: Implement

Now implement the feature directly in the worktree. Read the spec you just wrote and make all the changes described in it. Work in the worktree directory (`../mrmatt-{name}`), not the main repo.
