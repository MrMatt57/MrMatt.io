...
---
I am really enjoying the [switch](/posts/website-update-2016/) to a static Hugo site.  One of the main benefits is not having to run a server to host it.  That also means no `.htaccess` or `web.config` files to set headers or caching policies.  With a traditional site "static" assets can be safely cached and dynamic content will be served fresh each request.  Now that the whole site is static, we need to separate how things are cached.

This site\'s [toolchain](/stack/) uses s3cmd sync to deploy files to an Amazon S3 bucket that I use as an origin for KeyCDN.  In most CDNs the only way to vary cache control headers is to take the value from the origin.  So this means setting the headers in S3.  To do this, you can use s3cmd's to vary headers based on file types.  In my current implementation I am considering `html`, `xml`, and `json` files that may change and should not be browser cached.  Everything else I am caching for 24 hours.
...
### CDN settings
In your CDN set your Expire setting to honor your origin's server header.

Now static files; `css`, `js`, `fonts` and `images` are browser cached and only the main page is served for each request.
![Site Cache Profile](/img/CacheProfile.jpg)
### Bonus
Purge your KeyCDN cache with each deployment with an API call
```bash
